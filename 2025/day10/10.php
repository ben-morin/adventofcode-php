...
foreach ($F as $line)
{
    ...
    // part 2: using this method from /u/tenthmascot:
    // https://www.reddit.com/r/adventofcode/comments/1pk87hl/2025_day_10_part_2_bifurcate_your_way_to_victory/
    $part2 += solve($B, $jolt);
}

function patterns(array $B, int $num_vars): array
{
    // enumerate button subsets, compute sum pattern, group by parity,
    // keep the minimum button count per pattern...
    $num_buttons = count($B);
    $best = [];
    for ($mask = 0; $mask < (1 << $num_buttons); $mask++)
    {
        $pattern = array_fill(0, $num_vars, 0);
        $pressed = 0;
        for ($b = 0; $b < $num_buttons; $b++) if (($mask >> $b) & 1)
        {
            $pressed++;
            for ($v = 0; $v < $num_vars; $v++) $pattern[$v] += ($B[$b] >> $v) & 1;
        }
        $key = json_encode($pattern);
        if (!isset($best[$key]) || $best[$key] > $pressed)
            $best[$key] = $pressed;
    }
    $_res = [];
