...
Run with `docker`:
```
docker run --rm -it -v "$PWD":/usr/src/myapp -w /usr/src/myapp php:latest \
php 1.php 
```
```
part 1: 280
...
```
Running with `xdebug` disabled:
```
XDEBUG_MODE=off php 20.php
...
```
Running with `docker` and `opcache` enabled:
```
docker run --rm -it -v "$PWD":/usr/src/myapp -w /usr/src/myapp php:latest \
...
links
-
Advent of Code: [adventofcode.com](http://adventofcode.com)
<br>
